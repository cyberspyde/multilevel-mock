...
export async function POST(request: NextRequest) {
  try {
    ...
    try {
      ...
      // Use the same scoring logic for fallback
      const fallbackResponse = { summary, feedback };
      const score = estimateScore(session, fallbackResponse);
...
`;
  ...
  // ===== CONTENT SECTION =====
  if (isSpeaking && session.speakingAnswers?.length > 0) {
    ...
    session.speakingAnswers.forEach((answer: any, index: number) => {
      ...
      prompt += `Response: "${answer.transcription}"\n`;
      if (answer.duration) {
        prompt += `Duration: ${answer.duration} seconds\n`;
      }
      const wordCount = answer.transcription?.split(/\s+/).length || 0;
  ...
  } else if (isWriting && session.writingAnswers?.length > 0) {
    prompt += `WRITING RESPONSES:\n\n`;

    session.writingAnswers.forEach((answer: any, index: number) => {
      prompt += `Task ${index + 1}: ${answer.prompt?.prompt || 'Unknown'}\n`;
      prompt += `Word Count: ${answer.wordCount} words\n`;
      prompt += `Response:\n"${answer.content}"\n\n`;
    });
  }
